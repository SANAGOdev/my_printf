...
static int is_valid_flag(char const *format, int i)
{
    ...
    || format[i + 1] == 'u' || format[i + 1] == 'f' || format[i + 1] == 'b'
...
static int check_long_d(char const *format, int i, va_list ap)
{
    if (format[i] == '%') {
        if ((format[i + 1] == 'l') && (format[i + 2] == 'l')
        && (format[i + 3] == 'd')) {
            my_put_nbr(va_arg(ap, unsigned long long));
            return 4;
        }
        if (format[i + 1] == 'l' && format[i + 2] == 'd') {
            my_put_nbr(va_arg(ap, unsigned long));
            return 3;
        }
    }
    return 0;
